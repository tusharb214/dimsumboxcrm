...
// import { ArrowLeft, ShoppingBag, TrendingUp, BarChart3, Calendar, Download } from 'lucide-react';
...
//   const filteredDailyRevenue = dailyRevenue.filter(row => {
//     if (dailyFrom && row.date < dailyFrom) return false;
//     if (dailyTo && row.date > dailyTo) return false;
//     return true;
//   });
//   const paginatedDailyRevenue = filteredDailyRevenue.slice((dailyPage - 1) * DAILY_LIMIT, dailyPage * DAILY_LIMIT);

//   const downloadUserExcel = () => {
//     if (filteredDailyRevenue.length === 0) return toast.error('No data to download');
//     const rows = filteredDailyRevenue.map(row => ({
...
//     rows.push({
//       'Date': 'TOTAL',
//       'Revenue (₹)': filteredDailyRevenue.reduce((s, r) => s + r.revenue, 0),
//       'Orders': filteredDailyRevenue.reduce((s, r) => s + r.orderCount, 0),
//     });
...
//                 <div className="text-center py-8 text-slate-500 text-sm">No records in this date range</div>
//               ) : (
//                 <div className="rounded-xl border border-slate-700/40 overflow-hidden">
//                   <div className="divide-y divide-slate-800/60">
//                     {paginatedDailyRevenue.map(row => (
//                       <div key={row.date} className="flex items-center justify-between bg-slate-800/60 px-4 py-3">
//                         <div className="flex items-center gap-2">
...
//                     ))}
//                   </div>
//                   <Pagination page={dailyPage} total={filteredDailyRevenue.length} limit={DAILY_LIMIT} onChange={setDailyPage} />
//                 </div>
//               )}
//             </div>
//           )}
...
//           <div>
//             <div className="flex items-center justify-between mb-3">
//               <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Recent Orders</h3>
...
//                 type="date"
//                 value={orderFrom}
//                 onChange={e => { setOrderFrom(e.target.value); setOrderPage(1); }}
//                 className="input-field py-1.5 text-xs"
//               />
//               <label className="text-xs text-slate-500">To</label>
//               <input
//                 type="date"
//                 value={orderTo}
//                 onChange={e => { setOrderTo(e.target.value); setOrderPage(1); }}
//                 className="input-field py-1.5 text-xs"
//               />
//               {(orderFrom || orderTo) && (
//                 <button
//                   onClick={() => { setOrderFrom(''); setOrderTo(''); setOrderPage(1); }}
//                   className="text-xs text-slate-500 hover:text-white underline"
//                 >
//                   Clear
//                 </button>
...
//               <div className="text-center py-8 text-slate-500 text-sm">No orders in this date range</div>
//             ) : (
//               <div className="rounded-xl border border-slate-700/40 overflow-hidden">
...
//                           <td className="table-td font-mono text-xs text-sky-400">
//                             #{String(order.id).slice(-6).toUpperCase()}
//                           </td>
//                           <td className="table-td text-slate-400 text-xs">
//                             {new Date(order.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
//                           </td>
//                           <td className="table-td text-slate-400 text-xs">{order.items?.length || 0} items</td>
//                           <td className="table-td font-semibold text-white">{fmt(order.totalAmount || 0)}</td>
...
//                   </table>
//                 </div>
//                 <Pagination page={orderPage} total={sortedOrders.length} limit={ORDER_LIMIT} onChange={setOrderPage} />
//               </div>
//             )}
//           </div>
//         </div>
...
const AdminUserDetailPage: React.FC = () => {
  ...
  const totalRevenue = useMemo(() => deliveredOrders.reduce((s, o) => s + (o.totalAmount || 0), 0), [deliveredOrders]);
  ...
  const lastMonthRevenue = useMemo(() => {
    ...
    return deliveredOrders
      .filter(o => { const d = new Date(o.createdAt); return d.getMonth() === m.getMonth() && d.getFullYear() === m.getFullYear(); })
      .reduce((s, o) => s + (o.totalAmount || 0), 0);
  }, [deliveredOrders]);
  ...
  const barData = useMemo(() => {
    if (chartRange === 'monthly') {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        const filtered = deliveredOrders.filter(o => new Date(o.createdAt).getFullYear() === yr);
  ...
  const pieData = useMemo(() => {
    ...
    deliveredOrders.forEach(o => {
      (o.items || []).forEach((item: any) => {
        const name = item.materialName || 'Unknown';
        if (!map[name]) map[name] = { revenue: 0, qty: 0 };
        map[name].revenue += item.lineTotal ?? (item.priceAtOrder ?? item.price ?? 0) * item.quantity;
          ...
          {activeTab === 'daily' && (
            <div className="space-y-3">
              ...
              {filteredDaily.length === 0 ? (
                <div className="text-center py-10 text-slate-500 text-sm">No records in this date range</div>
              ) : (
                <div className="rounded-xl border border-slate-700/40 overflow-hidden">
                  <div className="divide-y divide-slate-800/60">
                    {paginatedDaily.map(row => {
                      ...
                      const dayOrders = userOrders.filter(o => {
                        const d = new Date(o.createdAt).toISOString().split('T')[0];
                        return d === row.date && ['DELIVERED', 'COMPLETED'].includes(o.status);
                      });
                      return (
                        <div key={row.date}>
                          <button
                            onClick={() => setExpandedDate(isOpen ? null : row.date)}
                            className="w-full flex items-center justify-between bg-slate-800/60 hover:bg-slate-800/80 px-4 py-3 transition-colors text-left"
                          >
                            <div className="flex items-center gap-2">
                            ...
                            <div className="bg-slate-900/60 px-4 py-3">
                              ...
                              {dayOrders.length > 0 ? (
                                <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
                                  {dayOrders.map(order => (
                                    <div key={order.id} className="bg-slate-800/50 rounded-lg px-3 py-2 space-y-1.5">
                                      <div className="flex items-center justify-between">
                                        <span className="font-mono text-xs text-sky-400">#{String(order.id).slice(-6).toUpperCase()}</span>
                                        <div className="flex items-center gap-2">
                                          <span className="text-xs font-semibold text-emerald-400">{fmt(order.totalAmount || 0)}</span>
                                          <StatusBadge status={order.status} />
                                        </div>
                                      </div>
                                      {order.items && order.items.length > 0 && (
                                        <div className="pl-2 space-y-1 border-l border-slate-700">
                                          {order.items.map((item: any, i: number) => (
                                            <div key={i} className="flex items-center justify-between gap-2">
                                              <p className="text-xs text-slate-400 truncate flex-1">
                                                {item.materialName || `Item ${i + 1}`}
                                                <span className="text-slate-600 ml-1">×{item.quantity || 1}</span>
                                              </p>
                                ...
                                <p className="text-xs text-slate-500 text-center py-2">No delivered orders this day</p>
                              )}
                            </div>
                          )}
                        </div>
