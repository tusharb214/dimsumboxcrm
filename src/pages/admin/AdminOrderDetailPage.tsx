...
const AdminOrderDetailPage: React.FC = () => {
  ...
  const markPaymentReceived = async () => {
    setUpdating(true);
    try { await adminApi.markPaymentReceived(String(order!.id)); toast.success('Payment marked as received!'); fetchOrder(); }
    catch (err: any) { toast.error(err?.response?.data?.message || 'Failed to mark payment received'); }
    finally { setUpdating(false); }
  };
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-black/60">Loading...</div>;
  if (!order) return <div className="flex items-center justify-center h-64 text-black/60">Order not found</div>;
    ...
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/admin/orders')} className="p-2 rounded-lg text-black/60 hover:text-black hover:bg-[#E3422C]/10 transition-all">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-xl font-bold text-black">Order #{String(order.id).slice(-8).toUpperCase()}</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E3422C] flex items-center gap-2">
              <Package className="w-4 h-4 text-sky-400" />
              <h2 className="text-sm font-semibold text-black">Order Items ({order.items?.length || 0})</h2>
            </div>

            {order.items?.length === 0 ? (
              <p className="text-sm text-black/60 text-center py-8">No items</p>
            ) : (
              <div className="max-h-[420px] overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-[#FFEEE7]/95 backdrop-blur z-10">
                    <tr className="text-xs text-black border-b border-[#E3422C]">
                      <th className="text-left font-medium px-5 py-2.5 w-10">#</th>
                      <th className="text-left font-medium px-2 py-2.5">Item</th>
                      <th className="text-center font-medium px-2 py-2.5 w-16">Qty</th>
                      <th className="text-right font-medium px-2 py-2.5 w-20">Price</th>
                      <th className="text-right font-medium px-5 py-2.5 w-24">Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#E3422C]/40">
                    {order.items?.map((item: any, i: number) => (
                      <tr key={i} className="hover:bg-[#E3422C]/5 transition-colors">
                        <td className="px-5 py-2.5 text-black/60">{i + 1}</td>
                        <td className="px-2 py-2.5">
                          <p className="font-medium text-black leading-tight">{item.materialName}</p>
                          <p className="text-xs text-black/60">
                            {item.category || ''}{item.brand ? ` • ${item.brand}` : ''}
                          </p>
                        </td>
                        <td className="px-2 py-2.5 text-center font-semibold text-black">x{item.quantity}</td>
                        <td className="px-2 py-2.5 text-right text-black/60">₹{item.priceAtOrder}</td>
                        <td className="px-5 py-2.5 text-right font-semibold text-sky-400">₹{item.lineTotal}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="px-5 py-4 border-t border-[#E3422C] bg-slate-900/50 flex items-center justify-between">
              <span className="text-sm font-semibold text-[#fff]">Total Amount</span>
          ...
          {(order.deliveryAddress || order.driverName || order.currentLocation) && (
            <div className="card px-5 py-4 space-y-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2 className="text-sm font-semibold text-black">Dispatch Info</h2>
              </div>
              ...
              {order.driverName && (
                <div className="flex gap-6">
                  <div>
                    <p className="text-xs text-black">Driver</p>
                    <p className="text-sm text-black">{order.driverName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-black">Vehicle</p>
                    <p className="text-sm text-black">{order.vehicleNumber || '—'}</p>
                  </div>
                  {order.estimatedDeliveryTime && (
                    <div>
                      <p className="text-xs text-black">ETA</p>
                      <p className="text-sm text-black">{order.estimatedDeliveryTime}</p>
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="space-y-4">
          ...
          <div className="card px-5 py-4 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-sm font-semibold text-[#141414]">Franchise Info</h2>
            </div>
            <div>
              <p className="text-xs text-slate-400">Name</p>
              <p className="text-sm font-semibold text-[#141414]">{order.userName || '—'}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400">Email</p>
               <p className="text-sm text-black">{order.userEmail || '—'}</p>
            </div>
          </div>
          ...
          <div className="card px-5 py-4 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-sm font-semibold text-black">Kitchen</h2>
            </div>
            {order.kitchenName ? (
              ...
              <>
                ...
                <Select
                  ...
                  onChange={(selected) => {
                    if (selected) {
                      assignKitchen(String(selected.value));
                    }
                  }}
                                   styles={{
                    control: (provided) => ({
                    ...
                    option: (provided, state) => ({
                      ...provided,
                      backgroundColor: state.isFocused ? "#ffeee7" : "#fff",
                      color: "#141414",
                      cursor: "pointer",
                    }),
          ...
          <div className="card px-5 py-4 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-black mb-1">Payment</h2>
              {order.paymentStatus && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${order.paymentStatus === 'CONFIRMED'
                    ? 'bg-emerald-500/20 text-emerald-400'
                    : 'bg-amber-500/20 text-amber-400'
                  }`}>
                  {order.paymentStatus === 'CONFIRMED' ? 'Received' : order.paymentStatus}
                </span>
              )}
            </div>
            ...
            <div className="flex justify-between border-t border-[#E3422C] pt-2">
            ...
            <div className="pt-2 border-t border-[#E3422C]">
              <p className="text-xs text-black mb-2">Payment Screenshot</p>
              {order.screenshotPath ? (
                <a href={getFileUrl(order.screenshotPath)} target="_blank" rel="noopener noreferrer">
                  <img
                    ...
                    className="max-h-56 w-full object-contain rounded-lg border border-slate-700"
                  />
                </a>
              ) : (
                <p className="text-xs text-black/60">User hasn't uploaded a screenshot yet.</p>
              )}
            </div>
            ...
            {order.paymentStatus !== 'CONFIRMED' && (
              <button
                ...
                disabled={updating}
                className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-semibold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 disabled:opacity-40 transition-all"
              >
