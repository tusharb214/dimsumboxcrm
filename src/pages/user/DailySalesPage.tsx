...
//     if (!reportDate)                    return toast.error('select the date ');
//     if (!totalSales || totalSales <= 0) return toast.error('enter the total sale');
//     const itemsSold = buildItemsSoldString();
//     if (!itemsSold)                     return toast.error('add at least one item');
//     setSubmitting(true);
//     try {
...
interface SaleItem { name: string; quantity: number; unitsPerPacket: number; }
...
const DailySalesPage: React.FC = () => {
  ...
  const [totalSales, setTotalSales]   = useState<number | ''>('');
  const [totalOrders, setTotalOrders] = useState<number | ''>('');
  const [items, setItems]             = useState<SaleItem[]>([{ name: '', quantity: 1, unitsPerPacket: 1 }]);
  const [submitting, setSubmitting]   = useState(false);
    ...
    materialApi.getAllMaterials()
      .then(r => {
        ...
        setMaterials(list.map((m: any) => ({
          name: m.name,
          unitsPerPacket: m.unitsPerPacket || 1,
        })));
      })
      .catch(() => {});
    fetchHistory(); fetchOrders(); fetchSummary();
  ...
  const stockItems = useMemo((): StockItem[] => {
    ...
    orders.filter(o => ['DELIVERED', 'COMPLETED'].includes(o.status)).forEach(o => {
      o.items?.forEach(item => {
        ...
        const units = item.quantity * (mat?.unitsPerPacket || 1);
        purchased[item.materialName] = (purchased[item.materialName] || 0) + units;
      });
    });
    const sold: Record<string, number> = {};
    salesHistory.forEach(log => {
  ...
  const downloadStockExcel = () => {
    ...
    const rows = filteredStock.map(s => ({
      'Item':         s.name,
      'Purchased (units)': s.purchased,
      'Sold (units)': s.sold,
      'Remaining (units)': s.remaining,
      'Status':       s.alertLevel.toUpperCase(),
      '% Remaining':  s.purchased > 0 ? Math.round((s.remaining / s.purchased) * 100) + '%' : '0%',
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    ws['!cols'] = [{ wch: 20 }, { wch: 18 }, { wch: 14 }, { wch: 18 }, { wch: 12 }, { wch: 14 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Stock Report');
    XLSX.writeFile(wb, `stock-report-${today}.xlsx`);
    toast.success('Excel downloaded!');
  ...
  const selectItem = (index: number, name: string) => {
    const mat = materials.find(m => m.name === name);
    setItems(prev => prev.map((item, idx) =>
      idx === index
        ? { ...item, name, unitsPerPacket: mat?.unitsPerPacket || 1 }
        : item
    ));
  };

  const addItem    = () => setItems(prev => [...prev, { name: '', quantity: 1, unitsPerPacket: 1 }]);
  const removeItem = (i: number) => setItems(prev => prev.filter((_, idx) => idx !== i));
  const updateItem = (i: number, field: keyof SaleItem, val: string | number) =>
    setItems(prev => prev.map((item, idx) => idx === i ? { ...item, [field]: val } : item));
  ...
  const buildItemsSoldString = () =>
    items.filter(i => i.name.trim() && i.quantity > 0)
         .map(i => `${i.name.trim()} x${i.quantity * i.unitsPerPacket}`).join(', ');

  const handleSubmit = async () => {
    if (!reportDate)                    return toast.error('enter report date');
    if (!totalSales || totalSales <= 0) return toast.error('enter valid total sales amount');
    const itemsSold = buildItemsSoldString();
    ...
    try {
      await salesApi.logDailySales({ reportDate, totalSales: Number(totalSales), totalOrders: Number(totalOrders) || 0, itemsSold });
      toast.success('Daily sales logged!');
      setTotalSales(''); setTotalOrders('');
      setItems([{ name: '', quantity: 1, unitsPerPacket: 1 }]);
      fetchSummary(); fetchHistory();
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Failed to log sales');
    } finally { setSubmitting(false); }
      ...
      {activeTab === 'log' && (
        <div className="card p-6 max-w-2xl">
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="text-xs text-slate-400">Items Sold (Packets)</label>
                <button onClick={addItem}
                  className="flex items-center gap-1 text-xs text-sky-400 hover:text-sky-300 transition-colors">
                  <Plus className="w-3.5 h-3.5" /> Add Item
                </button>
              ...
              <div className="space-y-2">
                {items.map((item, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <ItemSelect value={item.name} suggestions={materials.map(m => m.name)}
                    ...
                    {item.unitsPerPacket > 1 && (
                      <span className="text-xs text-slate-500 flex-shrink-0 whitespace-nowrap">
                        × {item.unitsPerPacket} = <span className="text-sky-400 font-semibold">{item.quantity * item.unitsPerPacket}</span>
                      </span>
                    )}
                    {items.length > 1 && (
                      <button onClick={() => removeItem(index)}
              ...
              {items.some(i => i.name.trim() && i.quantity > 0) && (
                <div className="mt-3 px-3 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700 space-y-1.5">
                  {items.filter(i => i.name.trim() && i.quantity > 0).map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      <span className="text-slate-300 font-medium">{item.name}</span>
                      <span className="text-slate-400">
                        {item.quantity} packets × {item.unitsPerPacket} =
                        <span className="text-sky-400 font-semibold ml-1">
                          {item.quantity * item.unitsPerPacket} units
                        </span>
                      </span>
                    </div>
                  ))}
                ...
                {paginatedStock.map(item => {
                  ...
                  return (
                    <div key={item.name} className="px-5 py-4">
                      ...
                      <div className="flex justify-between mt-1">
                        <p className="text-xs text-slate-600">Sold: {item.sold} units</p>
                        <p className="text-xs text-slate-600">{pct}% remaining</p>
                      </div>
                    </div>
                  );
